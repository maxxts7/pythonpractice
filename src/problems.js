...
function loadCustomProblems() {
  try {
    const raw = localStorage.getItem("pypractice-custom-problems");
...
export function saveCustomProblem(problem) {
  ...
  localStorage.setItem("pypractice-custom-problems", JSON.stringify(custom));
  // Update live array
  PROBLEMS.length = 0;
  PROBLEMS.push(...BUILTIN_PROBLEMS, ...custom);
}
...
export const PROBLEMS = [...BUILTIN_PROBLEMS, ...loadCustomProblems()];
